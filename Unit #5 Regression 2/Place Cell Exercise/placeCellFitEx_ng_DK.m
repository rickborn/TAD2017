...
spikeTrain = zeros(length(expTime),1);

for i=1:length(spikeTimes)
    spikeTrain = spikeTrain + (expTime == spikeTimes(i));
end
...
%% RESPONSE (Q3): The cell fires when the animal runs toward the far end of
% the maze. It is not just a place cell, since it is dependent on both
...
spikeHist = zeros(length(positionBins)-1,1);

for k = 2:length(positionBins)
    
    spikeHist(k) = sum(ratPosition(spikeIndex) < positionBins(k))...
                 - sum(ratPosition(spikeIndex) < positionBins(k-1));
...
% 'occupancyHist'
!!! YOUR CODE HERE
occupancyHist = zeros(length(positionBins)-1,1);  

for j = 2:length(positionBins)
    
    occupancyHist(j) = (sum(ratPosition < positionBins(j))...
                      - sum(ratPosition < positionBins(j-1)))/1000;  % Time points every millisecond
...
%% RESPONSE (Q4): 
% The overall trend is similar, but in the occupancy normalized
% distribution, the tails are gone and it is (expectedly) more normal.
...
% (dependent variable).
[b1,dev1,stats1] = glmfit(ratPosition, spikeTrain,'poisson');
lambdafit = glmval(b1, ratPosition, 'log');
figure(); plot(lambdafit);
...
%% RESPONSE (Q7):
% The residuals pretty closely track rat position, in that the residuals
% increase and decrease as the rat moves farther from and closer to its
...
%% RESPONSE (Q8):
% It only knows position. The model has no concept of whether direction is
% important, which we already know to be true based on the first plots of
% the data.
...
ratDirection = zeros(length(ratPosition),1);

for n=2:length(ratPosition)
    
    ratDirection(n) = (ratPosition(n)-ratPosition(n-1)) > 0;
    
end


% TODO: Add this to the model as another covariate:
[b3,dev3,stats3] = glmfit([ratDirection ratPosition ratPosition.^2], spikeTrain, 'poisson');

ratDirFit = glmval(b3, [ratDirection ratPosition ratPosition.^2], 'log');
figure(); plot(ratDirFit);
...
%% RESPONSE (Q18):
% Yes, in spades p=9.54215624147327e-20
...
%% RESPONSE (Q12):
% The more parameters, the better the fit. Adding a penalty discourages
% overfitting the model to too many parameters.
...
%% RESPONSE (Q14):
% Model 2 is a much better fit to the data than Model 1
...
% rate?
CI1 = [stats1.beta(1)-1.96*stats1.se(1) stats1.beta(1)+1.96*stats1.se(1);...
       stats1.beta(2)-1.96*stats1.se(2) stats1.beta(2)+1.96*stats1.se(2)]';
...
% RESPONSE (Q16):
% At x=0, lambda = b0. The 95% CI for b0 is [-7.7285 -7.1492].

% TODO: Compute the 95% CI for the parameters of Model 2.
CI2 = [stats2.beta(1)-1.96*stats2.se(1) stats2.beta(1)+1.96*stats2.se(1);...
       stats2.beta(2)-1.96*stats2.se(2) stats2.beta(2)+1.96*stats2.se(2);...
       stats2.beta(3)-1.96*stats2.se(3) stats2.beta(3)+1.96*stats2.se(3)]';
...
% RESPONSE (Q17):
% Yes, because the null (b3 = 0) is not contained within the 95% CI.
...
lambda3 = exp(b3(1) + b3(2)*ratDirection + b3(3)*ratPosition + b3(4)*ratPosition.^2);
logLikelihood3 = sum(log(poisspdf(spikeTrain,lambda3)));
AIC3 = -2 * logLikelihood3 + (2*4);
...
CI3 = [stats3.beta(2)-1.96*stats3.se(2) stats3.beta(2)+1.96*stats3.se(2)]; % I took "last parameter" to mean the direction coefficient
pBeta3 = stats3.p(2);	%... and significance level.

% QUESTION (Q20): What do these results tell us about our three models?
% Model 3 fits the data better than Model 2, which is better than Model 1
...
% RESPONSE (Q21): 
% Model 3 fits the data well, but Model 2 does not. The K-S plot for Model 
% 3 stays cleanly within the bounds of the 95% CI; whereas the Model 2 CDF 
% deviates significantly from the emperical CDF.