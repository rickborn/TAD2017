...
errorbar(mean(expt),sem(expt),'bo','MarkerFaceColor','b');
hold on
errorbar(mean(ctl),sem(ctl),'ro','MarkerFaceColor','r');
...
% Q20: Without making any post-hoc adjustments, on which tests is there a
% significant difference between the "experimental" and "control"
% treatments (at a significance level of p < 0.05)? (Check all that apply)

% A20: All p-values are below 0.05

% Q21:  Using the standard Bonferroni correction are all of the tests still
% significant at p < 0.05?
nTests = length(pVals);
BCpVals = pVals .* nTests;

% A21: No. Test #3 now gives a corrected p-value of 0.0578.

% Q22: Should you be concerned about multiple comparisons here?
% A22: Yes; we performed multiple hypothesis tests, and so we have a higher
...
% Q23: In the data provided in the .mat file, is there evidence that
% the tests results are correlated across patients?

[rhoExpt,pValExpt] = corr(expt);
% A23: Yes. In the experimental group, scores on all tests are correlated,
...
% Q24: How should your findings about the correlations of the tests adjust
% the way you think about correcting for the fact that you're performing
% multiple comparisons?

% A24: They are at least partially independent tests, so the probability of
...
% 0.05, although not necessarily as stringent as if they were independent.

% Q25: The authors cite your work in the statistics section as a
...
% A25: No. The two situations are quite different. Your paper asked "do the
...
% Q26: What is the probability of at least one false positive?
...
% Q27: What is the probability of producing a false positive if the tests
% are completely uncorrelated (i.e. sigma is 0 everywhere except the
% diagonal)?

% A27: There are two different ways to answer this question. The simplest
...
% Q28: What is the probability if all tests scores are perfectly correlated?

% A28: Again, two possible approaches. Simulation:
