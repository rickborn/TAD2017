...
% There are several other ways to compute binomial CIs, see:
% https://en.wikipedia.org/wiki/Binomial_proportion_confidence_interval
% The conceptually simplest is to just use the normal approximation:
pHat = 16/40;
n = 40;
nlCIhi = pHat + norminv(1 - myAlpha/2) * sqrt((pHat*(1-pHat))/n);
nlCIlo = pHat - norminv(1 - myAlpha/2) * sqrt((pHat*(1-pHat))/n);
% But this will be a bad approximation for small 'n' or as 'p' becomes
% close to 0 or 1.
...
if iSig
    pStim = 1 ./ (1 + exp(-(b(1) + b(2) + (b(3)+b(4)).*coh)));
else
    pStim = 1 ./ (1 + exp(-(b(1) + b(2) + b(3).*coh)));
end
plot(coh,pStim,'r-');

pNoStim = 1 ./ (1 + exp(-(b(1) + b(3).*coh)));
plot(coh,pNoStim,'k-');

% Note that we could also do the calculation using matrix multiplication.
% This is more convenient if we have many different covariates that, like
% 'coh' in our model, can take on a large range of values. The downside
% is that we then have to pass a column of 1's for the constant term and
% for any indicator variables, columns of 1's and 0's as appropriate. Here
% is how it would work:

% const = ones(size(coh));
% mstim = ones(size(coh));
% nomstim = zeros(size(coh));
% 
% % Trials with microstimulation
% if iSig
%     X = [const,mstim,coh,coh.*mstim];
% else
%     X = [const,mstim,coh];
% end
% pStim = 1 ./ (1 + exp(-(X*b)));
% plot(coh,pStim,'r-');
% 
% % Trials without microstimulation:
% if iSig
%     X = [const,nomstim,coh,coh.*nomstim];
% else
%     X = [const,nomstim,coh];
% end
% pNoStim = 1 ./ (1 + exp(-(X*b)));
...
tStr = sprintf('Effect Size = %.1f \xB1 %.1f', EVS2,seEffectSize);
text(xTxt,yTxt,tStr);
