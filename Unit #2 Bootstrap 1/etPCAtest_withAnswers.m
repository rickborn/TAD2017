% etPCAtest_withAnswers.m: Bootstrap calculation of standard error and
...
for k = 1:nBoot
    ...
    [~,~,bsLatent] = pca(bootSamp);
    thBoot(k) = bsLatent(1) / sum(bsLatent);
end

% standard error of thHat:
thSE = std(thBoot);
...
iLo = floor((myAlpha/2) * nBoot);   % index corresponding to lower bound
...
h2=line([thCIlow,thCIlow],[ax(3),ax(4)],'Color','r');
line([thCIhi,thCIhi],[ax(3),ax(4)],'Color','r');

legend([h2],'95% CI','Location','Northwest');
