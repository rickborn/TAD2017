...
% 
% They are very close, but a very slight difference may come from the fact
% the the percentile method chooses values from the sorted sample 
% variances that represent the 2.5th and 97.5th percentiles, 
% limiting the possible values to those in the dataset, while the normal
% method uses a normal inverse cumulative distribution function to
% calculate a deviance from an estimate of the standard error thHat. The
% interval is then the estiamte plus or minus this deviance. This
...
% The PCs are eigenvectors, which are still valid when sign-inverted. 
% The pattern matters more here, and as a somewhat aribtrary choice is 
% made regarding sign, we get mirror-image bimodality in the bootstrapped
% PCs.
...
% Ans: PC2 puts negative weights on the first 2 (closed book) tests and  
% positive weights on the 3 open-book tests, so loadPC2 is a contrast  
% between a student's open and close book performances. A student with a  
% high 'loadPC2' did much better on the open book tests than on the closed  
% book tests.
