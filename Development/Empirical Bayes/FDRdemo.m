...
% B. FDR is 1 - PPV, where PPV = P(H0 is true / H0 was rejected)
% C. Several handy features:
%    1)	FDR is calculated based on the p-values from the multiple tests
%    2)	more appropriate in cases where we expect H0 to be false for a fair 
%       number of features (e.g. genetic studies)
%    3)	If we choose an FDR of 0.05, it means that we would like no more 
%       than 5% of the features that we classify as significant to be false positives
%               a)	This is not the same as choosing the FWER to be 5%
%               b)	As with FWER, we can either use it to set a criterion 
%                   or we can use it to adjust our p-values
% D. FDR_i = (# of false positives) / (# called significant)
%   1) sort our 'n' p-values and find the ith ordered p-value (= p_i)
%   2) if we chose this p-value as the cut-off for significance, 
%      we would have i significant p-values
%   3) if all H0’s are true (uniform distribution of p-values), 
%      then the expected # of false positives would be: n x p_i 
%   4)	Thus FDRhat_i = n x p_i / i
%   5)	If all H0 are true, then FDR = FWER
%   6)	FDR is appropriate when a single falsely rejected H0 (i.e. false positive) 
...
% calculate the rejection line for qStar = 0.10
pValReject = (qStar .* iPvals) ./ nTotal;
h1 = plot(iPvals,pValReject,'r-');

% calculate the rejection line for qStar = 0.05
q05 = 0.05;
