...
% the total pool of possible residuals (use mdl1.Residuals.Raw), giving us
% errorStar (see below). For each of the nBoot iterations, we will draw the
% nPts residuals, which is the number of data points in our original
% dataset. We will add these residuals to the fitted linear predictor of
% the model to get yStar, which we can think of like a 'recomputation' of
% our data. Another way to think about this is yStar(i) = betaHat0x(i) +
% errorStar(i) Then, recompute the regression, only with yStar instead of
% the original Y. Store the regression coefficients from each run in a row
% of allBeta (allBeta should be a nBoot-x-2 matrix if completed correctly).
% We want to use 'regress' within our 'for' loop, because 'fitglm' is much
% slower (It is calculating a bunch of stuff that we don't need in the
% bootstrap.)
nBoot = 1000;
